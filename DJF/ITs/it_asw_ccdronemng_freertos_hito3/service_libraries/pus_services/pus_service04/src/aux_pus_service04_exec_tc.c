...
void pus_service4_exec_TC_4_1(tc_handler_t *ptc_handler) {
	...
	//La cosa es que el HK periódicamente va aumentando samples y hasta que no pides un report no hay endtime
	...
	for (uint16_t i = 0; (i < PUS_SERVICE4_MAX_NUM_OF_STATS) && (!error); i++){
		...
		if (StatsConfig.defined[i]) {
			...
			if (0 == error) {
				...
				start_time = StatsConfig.start_time[i]; //La primera a 0 y luego ya se va actualizando
				...
				error += pus_services_TM_X_Y_write_saved_PIDValue(&tm_handler,
						PID, &ParamStats[i].max); // El PID es para saber como interpretar el tipo de dato
...
// TC -> PID
	...
	if (error) {
     ...
     //LLEVAR IMPRESO EL 1.8 Y EL 1.7 y demás
			}
